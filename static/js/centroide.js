const CentroideMethods = {
  ...
  interpretarResultadosIA: async function () {
    ...
    const datosEntrada = {
      puntos: this.ultimosPuntos,
      estadisticas_puntos: {
        cantidad: this.ultimosPuntos.length,
        rango_x: {
          minimo: Math.min(...this.ultimosPuntos.map((p) => p.x)),
          maximo: Math.max(...this.ultimosPuntos.map((p) => p.x)),
        },
        rango_y: {
          minimo: Math.min(...this.ultimosPuntos.map((p) => p.y)),
          maximo: Math.max(...this.ultimosPuntos.map((p) => p.y)),
        },
        costos: {
          minimo: Math.min(...this.ultimosPuntos.map((p) => p.cost)),
          maximo: Math.max(...this.ultimosPuntos.map((p) => p.cost)),
          promedio: (this.ultimosPuntos.reduce((sum, p) => sum + p.cost, 0) / this.ultimosPuntos.length).toFixed(2),
          total: this.ultimosPuntos.reduce((sum, p) => sum + p.cost, 0),
        },
      },
      dispersion: {
        area_cobertura:
          (Math.max(...this.ultimosPuntos.map((p) => p.x)) - Math.min(...this.ultimosPuntos.map((p) => p.x))) *
          (Math.max(...this.ultimosPuntos.map((p) => p.y)) - Math.min(...this.ultimosPuntos.map((p) => p.y))),
      },
    }
    ...
    const resultadoEnriquecido = {
      ...
      analisis_ubicacion: {
        esta_dentro_del_area:
          this.ultimoCentroide.x >= datosEntrada.estadisticas_puntos.rango_x.minimo &&
          this.ultimoCentroide.x <= datosEntrada.estadisticas_puntos.rango_x.maximo &&
          this.ultimoCentroide.y >= datosEntrada.estadisticas_puntos.rango_y.minimo &&
          this.ultimoCentroide.y <= datosEntrada.estadisticas_puntos.rango_y.maximo,
        distancias_a_puntos: this.ultimosPuntos.map((punto) => ({
          nombre: punto.name,
          distancia: Math.sqrt(
            Math.pow(punto.x - this.ultimoCentroide.x, 2) + Math.pow(punto.y - this.ultimoCentroide.y, 2),
          ).toFixed(2),
          costo_transporte: (
            Math.sqrt(Math.pow(punto.x - this.ultimoCentroide.x, 2) + Math.pow(punto.y - this.ultimoCentroide.y, 2)) *
            punto.cost
          ).toFixed(2),
        })),
      },
      eficiencia: {
        costo_promedio_por_punto: (this.ultimoCosto / this.ultimosPuntos.length).toFixed(2),
        punto_mas_costoso: this.ultimosPuntos.reduce(
          (max, p) => {
            const distancia = Math.sqrt(
              Math.pow(p.x - this.ultimoCentroide.x, 2) + Math.pow(p.y - this.ultimoCentroide.y, 2),
            )
            const costoTransporte = distancia * p.cost
            return costoTransporte > max.costo ? { nombre: p.name, costo: costoTransporte.toFixed(2) } : max
          },
          { nombre: "", costo: 0 },
        ),
      },
  ...
  interpretarSensibilidadIA: async function () {
    if (!this.ultimosResultadosSensibilidad) {
      alert("❌ Primero debes realizar el análisis de sensibilidad antes de interpretarlo.")
      return
    }

    const containerId = "interpretacion-ia-sensibilidad"

    // Crear contenedor si no existe
    let container = document.getElementById(containerId)
    if (!container) {
      const resultadosDiv = document.getElementById("resultados-sensibilidad-centroide")
      container = document.createElement("div")
      container.id = containerId
      resultadosDiv.parentNode.insertBefore(container, resultadosDiv.nextSibling)
    }

    window.AIInterpreter.mostrarLoading(containerId, "🤖 Analizando sensibilidad con IA...")

    // Preparar datos contextuales más ricos
    const datosEntrada = {
      resultados: this.ultimosResultadosSensibilidad,
      estadisticas_puntos: {
        cantidad: this.ultimosPuntos.length,
        rango_x: {
          minimo: Math.min(...this.ultimosPuntos.map((p) => p.x)),
